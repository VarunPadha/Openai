...
state = SessionState.get(discussions=[{"role": "system", "content": "You are a helpful assistant."}])
...
if user_email in allowed_emails:
    ...
    if prompt.lower().strip() == "quit":
        state.discussions = [{"role": "system", "content": "You are a helpful assistant."}]
    ...
    if st.button("Generate Response"):
        if prompt.strip() == "":
            ...
            try:
                response, state.discussions = generate_text(prompt, 
                                                        api_key, 
                                                        state.discussions)
                st.text("AI Response:")
                st.write(response)
